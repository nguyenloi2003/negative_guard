...
function analyze_text_with_schema($text)
{
    ...
    // tiền xử lý heuristics sẵn có
    $pres  = _pre_screen_text($text);

    // Yêu cầu model TRẢ JSON THUẦN theo schema mô tả
    $rubric = <<<TXT
...
- Tham khảo gợi ý từ tiền xử lý (heuristic) sau nhưng vẫn ưu tiên bằng chứng gốc:
  FLAGS: %s
- TRẢ VỀ JSON THUẦN theo cấu trúc:
{
  "scores": {
    ...
    "propaganda": number,
    "scam_phishing": number,
    "hate_speech": number,
    "violence_threat": number
  },
  "severity_hint": number,
  "evidence_spans": {
    "misinformation":  string[],
    "propaganda":      string[],
    "scam_phishing":   string[],
    "hate_speech":     string[],
    "violence_threat": string[]
  },
  "rewrite_suggestion": string
}
Không thêm giải thích, không bao code fence.
TXT;
    ...
    $resp = openai_post('/chat/completions', [
        ...
        // Nếu nhà cung cấp hỗ trợ JSON mode, giữ response_format JSON để tăng độ chính xác
        'response_format' => ['type' => 'json_object'],
        'messages' => [
            ['role' => 'system', 'content' => 'Bạn là hệ thống đánh giá rủi ro, luôn trả JSON hợp lệ.'],
    ...
    // Lấy nội dung ra
    $out = _extract_text_from_response($resp); // fallback đã hỗ trợ choices[0].message.content
